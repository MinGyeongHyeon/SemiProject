...
/**
 * Servlet implementation class MissingBoardFileServlet
 */
@WebServlet("/missingfile.bo")
public class MissingBoardFileServlet extends HttpServlet {
	...
	System.out.println(a);
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			String re= multiRequest.getParameter("reward");
			int reward=0;
			if(re!=null) {
		...
		b.setMissPhone(phone);
			...
			int result = new MissingService().MissingInsert(b, fileList);
			
			if(result > 0) {
				response.sendRedirect(request.getContextPath() + "/missingListo.bo");
			}else {
				//실패시 저장된 사진 삭제
