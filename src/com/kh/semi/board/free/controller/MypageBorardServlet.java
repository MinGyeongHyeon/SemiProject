...
/**
 * Servlet implementation class MypageBorardServlet
 */
@WebServlet("/mypage.bo")
public class MypageBorardServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MypageBorardServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		User loginUser = (User) session.getAttribute("loginUser");
				...
				//전체 목록 갯수를 리턴받음
				int listCount = new UserBoardService().getmyListCount(loginUser.getUserNo());
				...
				request.getRequestDispatcher("views/member/7_member/3_mypage/5_writingHistory.jsp").forward(request, response);
	}
