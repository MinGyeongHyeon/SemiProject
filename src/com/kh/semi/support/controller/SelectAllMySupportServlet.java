...
@WebServlet("/mySupport.su")
public class SelectAllMySupportServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount = new SupportService().getSupportMoneyAllListCount(userNo);
		int listCount2 = new SupportService().getSupportProductAllListCount(userNo);
		...
		HashMap<String, Object> list = new SupportService().selectAllSupport(userNo, currentPage, currentPage2, limit, limit2);

		System.out.println("돈 : " + list.get("moneyList"));
		System.out.println("물품 : " + list.get("productList"));
		...
		request.getRequestDispatcher("views/member/7_member/3_mypage/6_supportHistory.jsp").forward(request, response);
	}
