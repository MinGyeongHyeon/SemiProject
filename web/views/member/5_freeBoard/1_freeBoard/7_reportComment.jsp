...
<table width="100%" height="300" border="0" cellspacing="0" cellpadding="0">
  <form id="report" name="report" method="post" action="<%= request.getContextPath() %>/InsertReportCom.ub" onsubmit="return check();">
  <tbody><tr>
		...
		<table border="0" cellspacing="0" cellpadding="0" class="choice">
			...
			<tr>
				...
				<td><input id="reason10" name="reason" type="radio" value="R4">기타</td>
						...
						<tr>
				<td><input type="hidden" value="<%= request.getParameter("commentNickNm")%>" name="commentNickNm"></td>
				<td><input type="hidden" value="<%= request.getParameter("commentNo")%>" name="commentNo"></td>
				<td><input type="hidden" value="<%= request.getParameter("loginUserNo")%>" name="loginUserNo"></td>
			</tr>
		</tbody></table>
					<p class="notice"><span class="red">자유 게시판에서는 <b>음란성 / 광고 / 욕설 / 비난 / 도배 를 엄격히 금지</b>합니다. 조금이라도 분란사유라고 생각되면 부담갖지 마시고 제발 신고 부탁 드립니다.</span></p>
